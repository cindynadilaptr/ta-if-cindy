...
\begin{lstlisting}[language=Python, title={\textbf{Kode 4.5} Pelaksanaan LOOCV, \textit{Grid Search}, dan Ekstraksi Evaluasi}, label={code:tuning}, frame=single, basicstyle=\ttfamily\scriptsize, breaklines=true]
...
Tahapan evaluasi dilakukan untuk mengukur besaran tingkat penyimpangan (\textit{error}) dan kesesuaian pola yang dihasilkan oleh model final terhadap keseluruhan populasi data. Sesuai dengan skema fase kedua, model final ini telah dilatih menggunakan 100\% observasi tanpa adanya data yang disembunyikan. Oleh karena itu, evaluasi pada tahap ini tidak bertujuan untuk menguji generalisasi pada data baru (\textit{testing}), melainkan untuk meninjau seberapa baik algoritma berhasil memetakan variabel fitur terhadap target aktualnya sebelum memasuki tahapan ekstraksi interpretasi SHAP. Pendekatan evaluasi dilakukan melalui perhitungan matematis antara nilai tebakan model (\texttt{y\_pred}) dan data realita (\texttt{y}). Metrik utama yang digunakan mencakup \textit{Mean Absolute Error} (MAE), \textit{Root Mean Squared Error} (RMSE), \textit{R-squared} ($R^2$), dan \textit{Mean Absolute Percentage Error} (MAPE). Rincian sintaksis komputasi untuk kalkulasi metrik tersebut dieksekusi pada Kode 4.8, di mana hasil rangkumannya dicatat pada Tabel \ref{table:4.hasil_evaluasi}.
...
\begin{longtable}{|c|p{0.55\textwidth}|c|}
    \caption{Hasil Evaluasi Performa Model Regresi Final}
    ...
    \hline
    \endlastfoot
    1 & \textit{Mean Absolute Error} (MAE) & 1,40 \tabularnewline 
    \hline
    2 & \textit{Root Mean Squared Error} (RMSE) & 1,77 \tabularnewline
    \hline
    3 & \textit{R-Squared} ($R^2$) & 0,9090 \tabularnewline
    \hline
    4 & \textit{Mean Absolute Percentage Error} (MAPE) & 7,22\% \tabularnewline
    \hline
\end{longtable}
...
Setelah meninjau sebaran kedekatan nilai prediksi secara visual pada tahapan sebelumnya, performa dari model referensi final perlu dikuantifikasi secara matematis menggunakan tolok ukur yang objektif. Kualitas model dalam mengenali pola data dikonfirmasi oleh perolehan matriks evaluasi pada Tabel \ref{table:4.hasil_evaluasi}. Skor \textit{Mean Absolute Error} (MAE) sebesar 1,40 mengindikasikan bahwa rata-rata selisih absolut antara tebakan sistem dengan realita prevalensi \textit{stunting} hanya terpaut sebesar 1,40\%. Nilai \textit{Root Mean Squared Error} (RMSE) sebesar 1,77, yang secara proporsional tidak terpaut jauh dari MAE, menunjukkan bahwa model cukup stabil dan terhindar dari penyimpangan residual yang ekstrem. Lebih lanjut, koefisien determinasi ($R^2$) sebesar 0,9090 membuktikan bahwa variabel prediktor yang digunakan secara kolektif mampu menjelaskan 90,9\% variansi prevalensi \textit{stunting}. Diperkuat dengan nilai \textit{Mean Absolute Percentage Error} (MAPE) sebesar 7,22\% yang berada di ambang batas sangat baik (di bawah 10\%), dapat disimpulkan bahwa model telah berhasil memetakan pola hubungan antar variabel secara komprehensif pada observasi populasi SSGI 2024.

Meskipun algoritma telah menunjukkan tingkat kesesuaian (\textit{fitting}) yang tinggi, metrik tersebut merepresentasikan evaluasi pada data pelatihan secara utuh, sehingga model final ini difokuskan sebagai model referensi interpretasi, bukan dideklarasikan untuk ekstrapolasi atau estimasi pada data baru. Kendala utama dari algoritma berbasis \textit{ensemble} seperti \textit{Random Forest} adalah sifatnya yang \textit{black-box}, sehingga mekanisme internal pembobotan keputusannya tidak dapat diobservasi secara langsung. Di sisi lain, rumusan rekomendasi penanganan \textit{stunting} yang presisi membutuhkan transparansi mengenai determinan mana yang paling berpengaruh. Oleh karena itu, tahapan analisis selanjutnya dikhususkan untuk menguraikan model final tersebut menggunakan pendekatan \textit{SHapley Additive exPlanations} (SHAP) guna mengekstraksi besaran dan arah kontribusi dari masing-masing fitur secara saintifik dan transparan.
...
Visualisasi \textit{Global Feature Importance} pada Gambar \ref{fig:shap_bar} menunjukkan besaran rata-rata kontribusi absolut dari setiap variabel prediktor. Berdasarkan hasil ekstraksi SHAP, variabel \texttt{persen\_tidak\_memenuhi\_mmff} atau persentase balita non-ASI yang tidak mendapatkan asupan susu minimal dua kali sehari (\textit{Minimum Milk Feeding Frequency}) menempati peringkat pertama sebagai determinan paling dominan. Secara keilmuan gizi, susu merupakan sumber utama protein hewani, kalsium, dan faktor pertumbuhan esensial yang sangat krusial bagi pembentukan linear tulang anak pada masa 1000 Hari Pertama Kehidupan (HPK) [SITASI 1]. Defisiensi asupan ini secara langsung memicu malnutrisi kronis yang bermanifestasi menjadi \textit{stunting}.

Faktor paling berpengaruh kedua dalam model adalah persentase kehamilan pada wanita dengan usia berisiko. Kehamilan pada usia yang belum matang secara biologis memicu terjadinya kompetisi pemenuhan nutrisi antara janin dengan tubuh ibu yang masih dalam tahap pertumbuhan. Kondisi ini secara signifikan meningkatkan probabilitas bayi lahir dengan Berat Badan Lahir Rendah (BBLR) serta panjang badan di bawah standar, yang merupakan prediktor kuat terjadinya \textit{stunting} di kemudian hari [SITASI 2]. 

Selanjutnya, variabel \texttt{persen\_tidak\_memenuhi\_mdd} atau kurangnya \textit{Minimum Dietary Diversity} (MDD) menempati posisi ketiga. Hal ini menegaskan bahwa kuantitas frekuensi makan tidak akan optimal jika tidak diimbangi dengan keberagaman jenis pangan. Kegagalan memenuhi variasi kelompok makanan harian dapat menyebabkan anak mengalami \textit{hidden hunger} (kelaparan tersembunyi), yaitu defisiensi mikronutrien spesifik seperti zink, zat besi, dan vitamin A yang sangat vital bagi metabolisme dan pertumbuhan seluler [SITASI 3].

Temuan dari algoritma ini memberikan wawasan (\textit{insight}) bahwa determinan proksimal yang berkaitan langsung dengan kualitas asupan gizi rumah tangga dan kesiapan biologis ibu memiliki bobot dampak yang lebih kuat terhadap prevalensi \textit{stunting} dibandingkan faktor infrastruktur atau fasilitas kesehatan. Akses terhadap fasilitas kesehatan cenderung bersifat sebagai faktor distal (tidak langsung), di mana keberadaannya tidak akan menurunkan angka \textit{stunting} secara optimal apabila akar permasalahan nutrisi harian tingkat individu tidak teratasi [SITASI 4]. Identifikasi peringkat ini menjadi landasan komprehensif untuk melakukan analisis yang lebih mendalam pada tahapan distribusi arah pengaruh variabel selanjutnya.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figure/summary.png} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figure/dependence_mmff.png} 
...
Pola lonjakan risiko yang teridentifikasi setelah melewati ambang batas 22\% tersebut mengonfirmasi tingginya kerentanan biologis balita non-ASI terhadap defisiensi protein hewani secara komunal. Secara keilmuan gizi dan pediatri, susu merupakan substitusi krusial yang menyumbangkan asam amino esensial, kalsium, dan mineral fosfor bagi anak yang tidak lagi menerima Air Susu Ibu (ASI). Ketiadaan asupan profil asam amino yang adekuat secara persisten terbukti secara klinis menekan sekresi \textit{Insulin-like Growth Factor 1} (IGF-1), yakni hormon peptida spesifik yang paling bertanggung jawab dalam menstimulasi proliferasi kondrosit dan pemanjangan tulang linear pada masa balita [SITASI 25]. Lebih lanjut, transisi drastis nilai SHAP menuju area pendorong risiko positif ketika persentase wilayah melampaui batas toleransi tersebut merepresentasikan titik kegagalan kompensasi gizi populasi. Apabila tingkat deprivasi asupan susu bergeser menjadi fenomena yang masif, mekanisme pertahanan tubuh balita tidak lagi mampu menutupi defisit gizi absolut melalui sumber pangan alternatif harian, sehingga bermanifestasi menjadi hambatan pertumbuhan massal [SITASI 26]. Visualisasi presisi dari algoritma ini secara saintifik memvalidasi urgensi pemenuhan asupan susu minimal sebagai jaring pengaman nutrisi wajib untuk mencegah malnutrisi kronis pada kelompok anak pasca-penyapihan.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figure/dependence_usia_ibu.png} 
...
Lonjakan risiko yang teramati ketika prevalensi usia ibu berisiko melampaui batas kritis tersebut sejalan dengan kajian biologi reproduksi dan obstetri. Kehamilan pada usia reproduksi ekstrem, khususnya pada fase remaja (di bawah 20 tahun), memicu terjadinya kompetisi biologis dalam penyerapan nutrisi esensial antara tubuh ibu yang masih dalam masa pertumbuhan linear dengan janin yang sedang dikandung (\textit{maternal-fetal competition}). Kompetisi ini secara langsung membatasi aliran oksigen dan transfer mikronutrien melalui plasenta, yang berdampak pada tingginya probabilitas \textit{Intrauterine Growth Restriction} (IUGR) dan kelahiran bayi dengan Berat Badan Lahir Rendah (BBLR) [SITASI 23]. Di sisi lain, kehamilan pada kelompok usia lanjut (di atas 35 tahun) turut membawa risiko penurunan fungsi organ reproduksi dan insufisiensi plasenta yang secara sistemik menghambat laju pertumbuhan janin secara intrauterin [SITASI 24]. Transisi nilai perhitungan matematis menuju area pendorong risiko positif secara masif setelah melewati ambang batas 14\% membuktikan bahwa beban akumulasi kelahiran bayi dengan kerentanan fisik sejak lahir dapat menciptakan efek bola salju (\textit{snowball effect}) yang memperburuk angka prevalensi secara komunal. Temuan ini secara saintifik memvalidasi urgensi optimalisasi program edukasi kesehatan reproduksi dan perencanaan keluarga untuk menjaga persentase wilayah di bawah ambang batas toleransi algoritma tersebut.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figure/dependence_mdd.png} 
...
Pola peningkatan risiko yang berbanding lurus dengan tingginya ketiadaan keberagaman pangan ini sejalan dengan patofisiologi malnutrisi kronis pada anak. Secara keilmuan gizi, ketergantungan pada pola makan monoton yang umumnya hanya didominasi oleh sumber karbohidrat tunggal akan memicu terjadinya defisiensi mikronutrien esensial multisistemik atau \textit{hidden hunger}. Ketidakmampuan dalam memenuhi spektrum nutrisi dari berbagai kelompok pangan mengakibatkan tubuh kekurangan kofaktor vital seperti zink, zat besi, dan vitamin A yang bertugas meregulasi sintesis hormon pertumbuhan, pembelahan sel osteoblas, serta diferensiasi jaringan fungsional pembentuk tulang [SITASI 21]. Lebih lanjut, transisi arah tarikan nilai SHAP menjadi positif secara masif ketika melewati ambang batas 55\% mengindikasikan bahwa suatu wilayah akan kehilangan efek daya lenting gizi (\textit{nutritional resilience}) apabila lebih dari separuh populasi balitanya terjebak dalam krisis variasi asupan harian [SITASI 22]. Pemetaan matematis dari algoritma ini secara saintifik memvalidasi urgensi penganekaragaman konsumsi pangan berbasis sumber daya lokal sebagai strategi krusial dalam pencegahan \textit{stunting} primer.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figure/dependence_pmt.png} 
...
Terdapat situasi non-linear yang ditangkap oleh model ini merepresentasikan karakteristik program Pemberian Makanan Tambahan (PMT) yang sejatinya berfungsi sebagai intervensi gizi spesifik yang bersifat kuratif dan komplementer, bukan sebagai substitusi asupan gizi harian. Secara keilmuan gizi kesehatan masyarakat, tingginya lonjakan risiko pada rentang ketiadaan PMT di bawah 85\% mengindikasikan bahwa pada tingkat tersebut, kelompok rentan sangat bergantung pada suplementasi eksternal untuk menutup defisit makronutrien (\textit{energy-protein gap}) yang tidak mampu terpenuhi secara mandiri oleh rumah tangga [SITASI 19]. Namun, ketika persentase ketiadaan bantuan menyentuh skala ekstrem (90\% hingga 95\%), daya ungkit variabel ini cenderung melemah dan terdistraksi. Hal ini secara epidemiologis dapat dijelaskan oleh pergeseran dominasi faktor determinan. Wilayah dengan tingkat kegagalan intervensi setinggi itu umumnya berhadapan dengan kerusakan sistemik pada infrastruktur kesehatan dan ketahanan pangan dasar. Dalam kondisi krisis multisektoral tersebut, keparahan prevalensi \textit{stunting} lebih didikte secara langsung oleh ketiadaan asupan pokok absolut harian (seperti MDD dan MMFF) maupun keparahan infeksi lingkungan, sehingga ketiadaan PMT tidak lagi menjadi variabel penentu utama yang membedakan peningkatan risiko di dalam algoritma [SITASI 20]. Dinamika matematis ini memvalidasi hipotesis bahwa efektivitas PMT sangat bergantung pada ketepatan sasaran distribusi (\textit{targeting accuracy}) di tengah kondisi ketahanan pangan yang bervariasi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figure/dependence_sanitasi.png} 
...
Korelasi linear antara ketiadaan fasilitas sanitasi yang layak dengan peningkatan risiko \textit{stunting} secara langsung berkaitan dengan tingginya paparan patogen lingkungan. Secara kajian kesehatan masyarakat, tingginya proporsi akses sanitasi berisiko memfasilitasi transmisi patogen fekal-oral yang memicu penyakit infeksi enterik berulang serta \textit{Environmental Enteric Dysfunction} (EED). Peradangan kronis pada saluran pencernaan akibat EED mengakibatkan atrofi vili usus, sehingga memicu malabsorpsi nutrisi esensial yang sejatinya sangat dibutuhkan untuk proses osteogenesis atau pembentukan tulang [SITASI 17]. Lebih lanjut, terbentuknya ambang batas kritis pada angka 10\% mengindikasikan berlakunya konsep perlindungan kelompok (\textit{herd protection}) dalam ekologi kesehatan lingkungan. Apabila persentase rumah tangga tanpa sanitasi layak melampaui batas toleransi tersebut, tingkat akumulasi kontaminasi patogen pada sumber air dan tanah akan mencapai beban paparan yang terlalu masif untuk dinetralisasi oleh sistem imun balita [SITASI 18]. Pemetaan ambang batas dari algoritma ini secara saintifik memvalidasi urgensi pemerataan infrastruktur sanitasi dasar sebagai prasyarat wajib agar intervensi gizi yang diberikan dapat diserap secara optimal oleh tubuh anak.
...
Kriteria pertama difokuskan pada kedekatan geografis dan keterwakilan regional Pulau Sumatera sebagai lokasi institusi tempat penelitian ini dilakukan. Provinsi Lampung dipilih sebagai representasi wilayah utama karena merupakan domisili pelaksanaan penelitian di Institut Teknologi Sumatera. Selain Lampung, Provinsi Aceh dan Sumatera Utara turut dipilih untuk mewakili karakteristik regional Sumatera masing-masing, yakni sebagai wilayah dengan angka prevalensi \textit{stunting} tertinggi dan wilayah dengan populasi terpadat di pulau tersebut yang mencapai lebih dari 15,4 juta jiwa [SITASI 1]. Peninjauan khusus pada ketiga provinsi ini bertujuan untuk memberikan kontribusi akademis yang relevan terhadap identifikasi dan penanganan masalah kesehatan di wilayah Sumatera.

Kriteria selanjutnya mencakup pemilihan provinsi yang memegang rekor ekstrem pada skala nasional untuk melihat respons model terhadap kondisi data yang kontras. Analisis pada wilayah dengan prevalensi tertinggi secara nasional, yakni Provinsi Nusa Tenggara Timur (NTT) dengan angka \textit{stunting} mencapai 37,0\% [SITASI 2], bertujuan untuk menemukan pemicu utama krisis gizi. Sementara itu, Provinsi Bali yang berhasil menekan prevalensi hingga menyentuh angka terendah secara nasional sebesar 8,7\% [SITASI 3] berfungsi sebagai pembanding keberhasilan intervensi. Provinsi Jawa Barat sebagai wilayah dengan populasi penduduk terpadat di tingkat nasional yang menembus lebih dari 50,4 juta jiwa [SITASI 4] juga disertakan untuk meninjau dampak persaingan akses sumber daya terhadap hasil prediksi. Penggabungan sampel regional dan nasional ini ditujukan agar visualisasi skala lokal dapat memberikan gambaran yang menyeluruh mengenai kinerja model \textit{machine learning}.
