...
\subsection{Persiapan Data} \label{subsec:penyiapan}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/penyiapan_data.pdf}
...
Alur pra-pemrosesan data ditunjukkan pada Gambar \ref{fig:5.flowchart_preprocessing}. Proses ini diawali dengan pemuatan \textit{dataset} ke dalam lingkungan komputasi serta pemeriksaan struktur dan konsistensi data. Selanjutnya dilakukan \textit{Exploratory Data Analysis} (EDA) untuk memperoleh gambaran umum karakteristik data. Lalu dilakukan proses pengecekan \textit{missing value} untuk memastikan apakah terdapat data yang hilang pada dataset penelitian. Tahap akhir pra-pemrosesan mencakup eliminasi variabel non-determinan serta penetapan prevalensi \textit{stunting} sebagai variabel target ($y$) dan indikator determinan sebagai variabel prediktor ($X$).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figure/preprocess.pdf}
    \caption{Alur Pra-pemrosesan Data}
    \label{fig:5.flowchart_preprocessing}
\end{figure}

\subsubsection{Representasi Dataset Penelitian} \label{sec:karakteristik_data}
Dataset yang diekstraksi bersumber dari Laporan Publikasi SSGI tahun 2024 yang diterbitkan oleh Kementerian Kesehatan Republik Indonesia \cite{kemenkes2025ssgi}. Data sekunder berskala nasional ini merepresentasikan berbagai indikator kesehatan, gizi, serta kondisi sosial ekonomi pada tingkat provinsi. Representasi struktur data perlu dijabarkan secara terperinci guna mendemonstrasikan transparansi pemilihan variabel sebelum diproses oleh algoritma.

Pada tahap ekstraksi awal, diperoleh format data terstruktur yang memuat 56 variabel independen dan satu variabel dependen berupa prevalensi \textit{stunting}. Tahapan pra-pemrosesan kemudian mengeksekusi penghapusan dua variabel berisiko \textit{data leakage}, yakni prevalensi \textit{underweight} dan prevalensi \textit{wasting}. Eliminasi ini pada akhirnya menyisakan 54 variabel independen final yang secara sah menjadi masukan bagi algoritma \textit{Random Forest}. Seluruh variabel prediktor tersebut dikelompokkan ke dalam kategori tematik berdasarkan karakteristik indikator yang diwakilinya. Pengelompokan ini dilakukan untuk mengorganisasi alur pemahaman data sekaligus menjaga konsistensi kerangka analisis determinan.

\begin{table}[H]
\centering
\small
\caption{Distribusi Variabel Determinan \textit{Stunting} berdasarkan Kategori}
\label{tab:distribusi_variabel}
\begin{tabular}{|c|p{0.45\textwidth}|c|c|}
\hline
\textbf{No} & \multicolumn{1}{c|}{\textbf{Kategori Variabel}} & \textbf{Sebelum Seleksi} & \textbf{Sesudah Seleksi} \tabularnewline \hline
1 & Riwayat Kehamilan dan Kelahiran & 15 & 15 \tabularnewline \hline
2 & Riwayat Pemberian ASI dan MP-ASI & 17 & 17 \tabularnewline \hline
3 & Akses dan Pemanfaatan Pelayanan Kesehatan & 9 & 9 \tabularnewline \hline
4 & Imunisasi dan Morbiditas Balita & 8 & 6 \tabularnewline \hline
5 & Kepemilikan dan Pemanfaatan Jaminan Kesehatan & 2 & 2 \tabularnewline \hline
6 & Pendampingan Keluarga dan Pengetahuan \textit{Stunting} & 4 & 4 \tabularnewline \hline
7 & Kontrasepsi Pasca Bersalin & 1 & 1 \tabularnewline \hline
8 & Bantuan Sosial & 1 & 1 \tabularnewline \hline
9 & Akses Air Minum dan Sanitasi & 4 & 4 \tabularnewline \hline
\multicolumn{2}{|c|}{\textbf{Total}} & \textbf{56} & \textbf{54} \tabularnewline \hline
\end{tabular}
\end{table}

Tabel \ref{tab:distribusi_variabel} memperlihatkan bahwa variabel determinan sangat didominasi oleh indikator riwayat kehamilan, pola pemberian makan, dan pemanfaatan layanan medis. Komposisi ini menegaskan pemakaian pendekatan multidimensi yang mengevaluasi faktor biologis, perilaku, serta lingkungan secara serentak di dalam sistem. Adapun rincian komprehensif mengenai pemetaan seluruh 56 variabel awal ke dalam sembilan kategori tematik tersebut dijabarkan secara lengkap pada bagian Lampiran. Pemetaan kategori ini mutlak diperlukan sebagai fondasi rujukan pada tahapan evaluasi berikutnya. Tahap lanjutan tersebut berpusat pada penelusuran arah kontribusi setiap fitur secara saintifik menggunakan pendekatan SHAP.


\subsection{Pemodelan \textit{Random Forest}} \label{subsec:modeling}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/alur_train.pdf}
...
Bersamaan dengan proses validasi LOOCV, dilakukan optimasi kinerja model melalui mekanisme \textit{Hyperparameter Tuning} menggunakan metode \textit{Grid SearchCV}. Tahap ini bertujuan untuk mencari kombinasi parameter terbaik dari algoritma Random Forest, meliputi jumlah pohon keputusan (\texttt{n\_estimators}), batas kedalaman maksimal pohon (\texttt{max\_depth}), jumlah sampel minimum pada simpul daun (\texttt{min\_samples\_leaf}), serta jumlah fitur maksimal yang dipertimbangkan pada setiap pemecahan simpul (\texttt{max\_features}), guna menghasilkan model dengan tingkat kesalahan prediksi terendah. Eksplorasi ruang parameter ini dinilai krusial untuk mencegah terjadinya \textit{overfitting}\footnote{\textit{Overfitting} merupakan kondisi saat model terlalu mempelajari \textit{noise} data latih sehingga kehilangan kemampuan generalisasi pada data baru.} yang sering menjadi kendala pada dataset berskala kecil \cite{fatmawati2024random}. Kombinasi parameter yang menghasilkan nilai rata-rata \textit{Root Mean Squared Error} (RMSE) terendah selama proses validasi silang kemudian ditetapkan sebagai konfigurasi model optimal.
...
\begin{enumerate}
    ...
    \item \textbf{\textit{R-squared} ($R^2$)}  
...
Interpretasi keputusan model \textit{Random Forest} yang bersifat \textit{black-box} dilakukan menggunakan metode SHAP guna mengidentifikasi besaran pengaruh faktor determinan secara transparan. Analisis mencakup tinjauan global skala nasional dan tinjauan lokal spesifik per provinsi. Alur kerja analisis interpretabilitas model ini diilustrasikan pada Gambar \ref{fig:7.flow_shap}.\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figure/alur_SHAP.pdf}
...
\begin{enumerate}
    ...
    Visualisasi ini merupakan bentuk distribusi nilai SHAP yang menyajikan arah pengaruh setiap variabel terhadap hasil prediksi. Grafik beeswarm menggunakan gradasi warna untuk membedakan besaran nilai asli dari sebuah observasi. Titik berwarna merah merepresentasikan nilai fitur yang tinggi, sedangkan titik biru merepresentasikan nilai fitur yang rendah. Posisi titik di sebelah kanan sumbu nol menandakan adanya dorongan peningkatan pada prediksi prevalensi \textit{stunting}. Sebaliknya, sebaran titik di sebelah kiri sumbu nol menunjukkan bahwa variabel tersebut membantu menurunkan angka tebakan \textit{stunting}.

    \item \textbf{\textit{Dependence Plot}}
    
    Visualisasi ini berbentuk grafik pencar (\textit{scatter plot}) yang bertugas membedah pola pengaruh dari satu variabel spesifik secara lebih rinci. Sumbu mendatar pada grafik ini menampilkan angka asli dari fitur yang sedang dianalisis. Sementara itu, sumbu vertikal menunjukkan besaran nilai SHAP yang dihasilkan oleh perubahan angka pada fitur tersebut. Visualisasi ini berguna untuk melihat ambang batas (\textit{threshold}) di mana sebuah indikator mulai merubah keputusan model secara drastis. Melalui grafik ini, peneliti dapat mengamati bentuk hubungan sebab-akibat secara langsung antara variabel masukan dan target keluaran.
\end{enumerate}
...
Mengingat interpretasi lokal bersifat mendalam dan kontekstual, tidak seluruh provinsi dianalisis secara individual. Oleh karena itu, dilakukan pemilihan sampel provinsi yang bertujuan untuk merepresentasikan variasi kondisi \textit{stunting} di Indonesia. Pemilihan sampel didasarkan pada distribusi nilai prevalensi \textit{stunting} hasil prediksi model, sehingga mencakup provinsi dengan tingkat prevalensi relatif tinggi, rendah, serta kondisi menengah. Hal ini didasarkan pada distribusi nilai prevalensi \textit{stunting} hasil prediksi model, yakni dengan memilih secara spesifik provinsi yang memiliki tingkat prevalensi tertinggi (mewakili nilai maksimum), tingkat prevalensi terendah (mewakili nilai minimum), serta tingkat prevalensi menengah (mewakili nilai median). Pendekatan ini memungkinkan analisis yang lebih terfokus tanpa mengurangi representativitas karakteristik wilayah secara nasional.

\subsubsection{Visualisasi dan Analisis \textit{Waterfall Plot}}
Setelah sampel provinsi ditetapkan, interpretasi lokal dilakukan menggunakan visualisasi \textit{Waterfall Plot}. Visualisasi ini menampilkan bagaimana nilai prediksi suatu provinsi terbentuk secara bertahap mulai dari nilai dasar (\textit{base value}) hingga mencapai nilai prediksi akhir. Nilai dasar tersebut merepresentasikan rata-rata prediksi model pada seluruh data, kemudian setiap variabel determinan ditambahkan atau dikurangkan secara berurutan sesuai besar kontribusinya terhadap prediksi provinsi tersebut. Dengan demikian, grafik ini memperlihatkan proses pembentukan prediksi sebagai akumulasi kontribusi dari masing-masing variabel.

Secara visual, \textit{Waterfall Plot} menyerupai diagram batang bertingkat yang bergerak naik atau turun dari satu titik awal menuju nilai akhir. Batang yang mengarah ke atas menunjukkan variabel yang meningkatkan nilai prediksi prevalensi stunting, sedangkan batang yang mengarah ke bawah menunjukkan variabel yang menurunkan nilai tersebut. Panjang setiap batang menggambarkan besar kecilnya pengaruh variabel terhadap hasil prediksi. Melalui penyajian ini, pembaca dapat secara langsung melihat faktor mana yang paling dominan membentuk nilai prediksi pada suatu provinsi, sehingga keputusan model tidak lagi terlihat sebagai proses yang tertutup atau sulit dipahami.

\section{Lingkungan Pengembangan Model}
Proses pembangunan model dan pengolahan data dilakukan dalam lingkungan bahasa pemrograman \texttt{Python} versi 3.10. Untuk menjaga reproduksibilitas penelitian, berikut adalah daftar pustaka (\textit{library}) utama beserta versinya dan peran spesifiknya dalam sistem:

\begin{enumerate}
    \item \texttt{scikit-learn} (v1.7.2), digunakan untuk implementasi algoritma \textit{Random Forest}, pembagian data \textit{Leave-One-Out Cross-Validation}, serta optimasi \textit{hyperparameter}.
    \item \texttt{shap} (v0.49.1), digunakan sebagai instrumen interpretasi model untuk menghitung nilai kontribusi tiap fitur melalui pendekatan \textit{Shapley values}.
    \item \texttt{pandas} (v2.3.2), digunakan untuk manipulasi struktur data, khususnya dalam proses agregasi dari data individu ke tingkat provinsi.
    \item \texttt{numpy} (v2.3.3), mendukung operasi komputasi matriks dan pemrosesan \textit{array} numerik yang efisien.
    \item \texttt{matplotlib} (v3.10.6) \& \texttt{seaborn} (v0.13.2), digunakan untuk memvisualisasikan distribusi variabel dan hasil interpretasi fitur secara grafis.
    \item \texttt{scipy} (v1.16.2), digunakan untuk pengolahan statistik tingkat lanjut yang diperlukan dalam pembersihan data.
\end{enumerate}

\section{Ilustrasi Perhitungan Metode} \label{sec:ilustrasi_metode}

Bagian ini menyajikan simulasi pemrosesan data SSGI 2024 oleh model untuk menghasilkan prediksi prevalensi \textit{stunting}. Angka yang digunakan di sini adalah data hipotetis untuk menggambarkan alur kalkulasi dari \textit{input} fitur hingga evaluasi akhir.

\subsection{Prediksi \textit{Random Forest Regression}}

Dalam penelitian ini, prediksi prevalensi \textit{stunting} untuk satu provinsi tidak ditentukan oleh satu pohon keputusan tunggal, melainkan melalui konsensus dari banyak pohon (\textit{ensemble}). Setiap pohon dalam model akan memberikan estimasi angka prevalensi berdasarkan fitur \textit{input} yang dipelajarinya.
...
\subsection{Perhitungan \textit{Error} Prediksi}
...
\par\vspace{0.2cm}
\begin{align*}
    \text{RMSE} &= \sqrt{\frac{4 + 1 + 4}{3}} \\[0.3cm]
...
Bagian ini menjabarkan skenario eksperimen yang dilakukan untuk melatih dan memvalidasi model. Mengingat jumlah observasi yang terbatas yaitu provinsi di Indonesia, strategi pembagian data dan pemilihan parameter menjadi krusial untuk mencegah \textit{overfitting} dan memastikan hasil evaluasi yang objektif.

\subsection{Skema \textit{Leave-One-Out Cross Validation} (LOOCV)}

Penelitian ini menerapkan metode \textit{Leave-One-Out Cross Validation} (LOOCV) untuk memaksimalkan penggunaan data. Dalam skema ini, evaluasi dilakukan melalui \textit{N} kali iterasi, di mana pada setiap iterasi, satu provinsi disisihkan sebagai data uji (\textit{testing set}) sementara sisa provinsi lainnya digunakan sebagai data latih (\textit{training set}).
...
\begin{table}[H]
    ...
    \begin{tabular}{cll}
    \toprule
    \textbf{Iterasi} & \textbf{Data Latih (N-1 Provinsi)} & \textbf{Data Uji (1 Provinsi)} \\
    \midrule
    1 & Prov 2, 3, ..., N & \textbf{Provinsi 1} \\
    2 & Prov 1, 3, ..., N & \textbf{Provinsi 2} \\
    ... & ... & ... \\
    N & Prov 1, 2, ..., N & \textbf{Provinsi N} \\
    \bottomrule
    \end{tabular}
\end{table}
